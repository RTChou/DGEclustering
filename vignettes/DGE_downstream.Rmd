...
cluster.out <- 'intego_time_series_padj_multiple_0.9.tsv'
...
dat <- read.table('test_all_sig_genes.tsv', header=TRUE, check.names=FALSE, sep='\t', stringsAsFactors=FALSE)
bg.genes <- read.table(file1, header=TRUE, sep='\t', stringsAsFactors=FALSE)[gene.col]
ann <- annotate.genes(OrgDb=org.db, keyType=key.type, genes=dat$gene.col, GOEnrichment=FALSE, BgGenes=bg.genes)
```
